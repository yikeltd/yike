...
**YIP Runtime override — ACTIVE, FINAL INFRASTRUCTURE LAYER (2026-07-26):** Founder-approved `CapabilityRuntime` — the OS façade on top of `PluginHost` (discovery, manifests, permissions, providers, config, metrics, health, soft try/catch-timeout-circuit-breaker sandbox, one diagnostics snapshot). Reuses `PluginHost` and `plugins/dependency.ts` unmodified — no second lifecycle engine, no second dependency resolver. Still **no ML/LLM**; no filesystem dynamic loading; no DB-stored manifests. **This is the last platform/infrastructure layer YIP gets — do not approve another orchestration layer above it.** All future YIP work (VIN decoding, flood-risk scoring, real pricing/trust, etc.) ships as capabilities (plugins) on top of this runtime, not as new infrastructure. See [YIP_RUNTIME.md](../architecture/YIP_RUNTIME.md) · [YIP_RUNTIME_IMPLEMENTATION.md](../implementation/YIP_RUNTIME_IMPLEMENTATION.md).
