...
export async function POST(req: Request, ctx: RouteCtx) {
  ...
  const body = (await req.json()) as { status: VerifierStatus; notes?: string };

  const allowed: VerifierStatus[] = [
    "approved",
    "paused",
    "suspended",
    "inactive",
  ...
  await admin.from("field_verifiers").update(patch).eq("id", id);

  const hdrs = await headers();
  await writeAuditLog({
    actor_id: auth.user.id,
    actor_role: auth.profile.role,
    action: "verifier.suspend",
