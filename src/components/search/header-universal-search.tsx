    ...
    <div ref={wrapRef} className={cn("relative min-w-0 flex-1", className)}>
      <div
        className={cn(
          ...
          isDesktop && "border-navy/10 bg-navy/[0.04]",
          !isHero && !isDesktop && "border-navy/10 bg-navy/[0.04] shadow-sm",
          open && "border-gold/50 shadow-glow-gold ring-2 ring-gold/20",
        )}
      >
        <Search
          className={cn(
            "ml-3 shrink-0 text-navy/45",
            isLarge ? "h-4.5 w-4.5 h-[18px] w-[18px]" : "h-4 w-4",
          )}
          aria-hidden
        />
        <input
          ...
          className={cn(
            "min-w-0 flex-1 bg-transparent py-0 pr-2 text-navy outline-none placeholder:text-navy/45",
            isLarge ? "h-11 text-[15px]" : "h-9 text-sm",
          )}
