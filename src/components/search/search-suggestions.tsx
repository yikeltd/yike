...
export function SearchSuggestions({
  ...
  if (isEmpty) {
    return (
      <section className="w-full space-y-5 px-3 py-4 lg:px-6 xl:px-8">
        {recent.length > 0 ? (
            ...
            <div className="hide-scrollbar flex gap-2 overflow-x-auto pb-0.5">
              {recent.map((r) => (
                <button
                  ...
                  className="pressable shrink-0 rounded-full border border-dashed border-navy/15 bg-white px-3.5 py-2 text-xs font-semibold text-navy"
                >
                  {r.label}
                </button>
              ))}
        ...
        <PopularSearchChips
          ...
          items={PROPERTY_POPULAR_SEARCHES}
        />
        ...
        <div>
          ...
          <CategoryBrowseGrid items={PROPERTY_CATEGORY_BROWSE} />
        </div>
  ...
  return (
    <section className={cn("w-full px-3 pb-4 pt-2 lg:px-6 xl:px-8")}>
