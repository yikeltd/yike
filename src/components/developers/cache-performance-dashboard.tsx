...
import { CacheZoneOverview } from "./cache/cache-zone-overview";
import { CachePurgeConsole } from "./cache/cache-purge-console";
import { EdgePerformanceMetrics } from "./cache/edge-performance-metrics";
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-black text-navy dark:text-white flex items-center gap-2">
                ...
                Redis Caching & Edge Performance Hub
              </h1>
              <p className="text-xs text-navy/60 dark:text-white/60 mt-1">
                Multi-layer Redis cache zone management, pattern invalidations, global CDN POP edge latencies, and payload compression.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 px-3.5 py-2 text-xs font-black text-emerald-600 dark:text-emerald-400 shrink-0">
              ...
              <span>EDGE CDN ACTIVE (97.9% CACHE HIT RATE)</span>
            </div>
          </div>
        </div>

        {/* 1. CACHE ZONE OVERVIEW */}
        <CacheZoneOverview />

        {/* 2. CACHE PURGE & PATTERN EVICTION CONSOLE */}
        <CachePurgeConsole />

        {/* 3. EDGE CDN & COMPRESSION METRICS */}
