...
import { SecurityScoreCard } from "./security/security-score-card";
import { VulnerabilityScanner } from "./security/vulnerability-scanner";
import { SecretManagerConsole } from "./security/secret-manager-console";
  ...
  return (
    <div className="min-h-[100dvh] bg-[#f7f9fc] dark:bg-[#021433] text-navy dark:text-white py-8 px-4 sm:px-6 select-none">
      <div className="mx-auto max-w-6xl space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-black text-navy dark:text-white flex items-center gap-2">
                ...
                Enterprise Security Center & Vulnerability Auditor
              </h1>
              <p className="text-xs text-navy/60 dark:text-white/60 mt-1">
                OWASP Top 10 automated security checks, Supabase RLS policy auditing, secret rotation schedules, and security score.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 px-3.5 py-2 text-xs font-black text-emerald-600 dark:text-emerald-400 shrink-0">
              ...
              <span>GRADE A+ ENTERPRISE CERTIFIED (98/100)</span>
            </div>
          </div>
        </div>
        ...
        {/* 3. SECRET MANAGER CONSOLE */}
        <SecretManagerConsole />
